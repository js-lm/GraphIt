...
RAYGUIAPI int GuiButton(Rectangle bounds, const char *text, float sclaeFactor = 1.0f);                                           // Button control, returns true when clicked
...
int GuiButton(Rectangle bounds, const char *text, float sclaeFactor)
{
    int result = 0;
    GuiState state = guiState;
    ...
    if ((state != STATE_DISABLED) && !guiLocked && !guiSliderDragging)
    {
        ...
        // Check button state
        if (CheckCollisionPointRec(mousePoint, {bounds.x * sclaeFactor, bounds.y * sclaeFactor, bounds.width * sclaeFactor, bounds.height * sclaeFactor}))
        {
            if (IsMouseButtonDown(MOUSE_LEFT_BUTTON)) state = STATE_PRESSED;
            else state = STATE_FOCUSED;
